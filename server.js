...
local function apiGet(url)
    local ok, res = pcall(function() return http({ Url = url, Method = "GET", Headers = { ["Content-Type"] = "application/json" } }) end)
    if ok and res and res.Body then return res.Body end
    return nil
end
local function apiPost(url, data)
    local ok, res = pcall(function()
        return http({ Url = url, Method = "POST", Headers = { ["Content-Type"] = "application/json" }, Body = HttpService:JSONEncode(data) })
    end)
    if ok and res and res.Body then return res.Body end
    return nil
end
local function copyToClipboard(text)
    pcall(function()
        if setclipboard then setclipboard(text) elseif syn and syn.setclipboard then syn.setclipboard(text) elseif toclipboard then toclipboard(text) end
    end)
end
local guiParent
if gethui then local ok, r = pcall(gethui); if ok and r then guiParent = r end end
guiParent = guiParent or CoreGui
for _, c in ipairs(guiParent:GetChildren()) do if c.Name == "XenoPanel" then pcall(c.Destroy, c) end end
...
local Theme = { Bg = Color3.fromRGB(0, 0, 0), Border = Color3.fromRGB(48, 38, 55), Orange = Color3.fromRGB(255, 140, 40), Red = Color3.fromRGB(240, 80, 90), Green = Color3.fromRGB(80, 220, 130), Gray = Color3.fromRGB(100, 100, 120), Text = Color3.fromRGB(245, 235, 245), Dim = Color3.fromRGB(160, 145, 175), Blue = Color3.fromRGB(70, 130, 255) }
local function create(cn, props) local i = Instance.new(cn); for k, v in pairs(props) do i[k] = v end; return i end
local function corner(p, r) return create("UICorner", { CornerRadius = UDim.new(0, r), Parent = p }) end
local function stroke(p, c, t, tr) return create("UIStroke", { Color = c or Theme.Border, Thickness = t or 1, Transparency = tr or 0, ApplyStrokeMode = Enum.ApplyStrokeMode.Border, Parent = p }) end
local isMobile = UserInputService.TouchEnabled
local FRAME = isMobile and { X = 280, Y = 380 } or { X = 460, Y = 540 }
local MainFrame = create("Frame", { Parent = ScreenGui, AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.new(0.5, 0, 0.5, 0), Size = UDim2.new(0, FRAME.X, 0, FRAME.Y), BackgroundColor3 = Theme.Bg, BorderSizePixel = 0, ClipsDescendants = true })
corner(MainFrame, 12); stroke(MainFrame, Theme.Border, 1, 0.3)
local StarContainer = create("Frame", { Parent = MainFrame, Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, ZIndex = 1 })
local stars = {}
for i = 1, (isMobile and 22 or 38) do
    local sz = math.random(1, isMobile and 3 or 4)
    local star = create("Frame", { Parent = StarContainer, Size = UDim2.new(0, sz, 0, sz), Position = UDim2.new(math.random() * 0.95, 0, math.random(), 0), BackgroundColor3 = Color3.fromRGB(255, 255, 255), BackgroundTransparency = math.random(30, 70) / 100, BorderSizePixel = 0, ZIndex = 2 })
    corner(star, sz)
...
task.spawn(function()
    while ScreenGui.Parent do
        for _, s in ipairs(stars) do
            ...
            if ny > 1 + s.fall then ny = -0.1; s.ui.Position = UDim2.new(math.random() * 0.95, 0, ny, 0); s.ui.BackgroundTransparency = math.random(30, 70) / 100
            else s.ui.Position = UDim2.new(s.ui.Position.X.Scale, 0, ny, 0) end
        end
        task.wait(0.05)
    end
end)
task.spawn(function()
    local up, p = true, 0
    while ScreenGui.Parent do
        if up then p = p + 0.03; if p >= 1 then up = false end
        else p = p - 0.03; if p <= 0.3 then up = true end end
        local st = MainFrame:FindFirstChildOfClass("UIStroke")
        if st then st.Transparency = 0.3 + p * 0.4 end
        task.wait(0.05)
    end
end)
local Header = create("Frame", { Parent = MainFrame, Size = UDim2.new(1, 0, 0, isMobile and 38 or 44), BackgroundColor3 = Theme.Bg, BorderSizePixel = 0, ZIndex = 10 })
corner(Header, 12)
local Title = create("TextLabel", { Parent = Header, BackgroundTransparency = 1, AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.new(0.5, 0, 0.5, 0), Size = UDim2.new(0.7, 0, 1, 0), Text = "REMOTE CONTROL PANEL", TextColor3 = Theme.Text, TextSize = isMobile and 12 or 14, Font = Enum.Font.GothamBold, ZIndex = 12 })
local CountLabel = create("TextLabel", { Parent = Header, BackgroundTransparency = 1, AnchorPoint = Vector2.new(0, 0.5), Position = UDim2.new(0, 12, 0.5, 0), Size = UDim2.new(0, 120, 1, 0), Text = "0/0", TextColor3 = Theme.Orange, TextSize = isMobile and 11 or 12, Font = Enum.Font.GothamBold, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 12 })
local CloseBtn = create("TextButton", { Parent = Header, AnchorPoint = Vector2.new(1, 0.5), Position = UDim2.new(1, -10, 0.5, 0), Size = UDim2.new(0, isMobile and 22 or 26, 0, isMobile and 22 or 26), BackgroundColor3 = Theme.Bg, Text = "X", TextColor3 = Color3.fromRGB(200, 200, 200), TextSize = isMobile and 12 or 14, Font = Enum.Font.GothamBold, AutoButtonColor = false, ZIndex = 12 })
corner(CloseBtn, 6); stroke(CloseBtn, Theme.Border, 1, 0.3)
CloseBtn.MouseEnter:Connect(function() TweenService:Create(CloseBtn, TweenInfo.new(0.15), { BackgroundColor3 = Theme.Red, TextColor3 = Color3.fromRGB(255, 255, 255) }):Play() end)
CloseBtn.MouseLeave:Connect(function() TweenService:Create(CloseBtn, TweenInfo.new(0.15), { BackgroundColor3 = Theme.Bg, TextColor3 = Color3.fromRGB(200, 200, 200) }):Play() end)
CloseBtn.MouseButton1Click:Connect(function() ScreenGui:Destroy() end)
do
    local dragging, dragStart, startPos, dragInput = false, nil, nil, nil
    local function beginDrag(input) dragging = true; dragStart = input.Position; startPos = MainFrame.Position; dragInput = input end
    Header.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then beginDrag(input) end end)
...
local SearchContainer = create("Frame", { Parent = MainFrame, Position = UDim2.new(0, 10, 0, SearchY), Size = UDim2.new(1, -20, 0, SearchH), BackgroundColor3 = Theme.Bg, BorderSizePixel = 0, ZIndex = 10 })
corner(SearchContainer, 8); stroke(SearchContainer, Theme.Border, 1, 0.3)
create("TextLabel", { Parent = SearchContainer, BackgroundTransparency = 1, Position = UDim2.new(0, 10, 0.5, 0), AnchorPoint = Vector2.new(0, 0.5), Size = UDim2.new(0, 18, 0, 18), Text = "🔍", TextColor3 = Theme.Dim, TextSize = isMobile and 12 or 14, Font = Enum.Font.Gotham, ZIndex = 11 })
local SearchBox = create("TextBox", { Parent = SearchContainer, Position = UDim2.new(0, 34, 0, 0), Size = UDim2.new(1, -44, 1, 0), BackgroundTransparency = 1, PlaceholderText = "Search username, display, or brainrot...", Text = "", TextColor3 = Theme.Text, PlaceholderColor3 = Theme.Dim, TextSize = isMobile and 12 or 13, Font = Enum.Font.GothamMedium, TextXAlignment = Enum.TextXAlignment.Left, ClearTextOnFocus = false, ZIndex = 11 })
local ListY = SearchY + SearchH + 8
local Results = create("ScrollingFrame", { Parent = MainFrame, Position = UDim2.new(0, 8, 0, ListY), Size = UDim2.new(1, -16, 1, -(ListY + 8)), BackgroundTransparency = 1, BorderSizePixel = 0, ScrollBarThickness = 3, ScrollBarImageColor3 = Theme.Orange, ScrollBarImageTransparency = 0.5, ZIndex = 5 })
local PlayerContainer = create("Frame", { Parent = Results, Size = UDim2.new(1, 0, 0, 0), BackgroundTransparency = 1, AutomaticSize = Enum.AutomaticSize.Y })
local ListLayout = create("UIListLayout", { Parent = PlayerContainer, Padding = UDim.new(0, 6), SortOrder = Enum.SortOrder.LayoutOrder })
local EmptyLabel = create("TextLabel", { Parent = PlayerContainer, Size = UDim2.new(1, 0, 0, 60), BackgroundTransparency = 1, Text = "waiting for clients...", TextColor3 = Theme.Dim, TextSize = isMobile and 12 or 13, Font = Enum.Font.GothamMedium, LayoutOrder = -1, ZIndex = 5 })
local function rowHeight() return isMobile and 132 or 116 end
local function makeChip(parent, label, color, x, w, onClick)
    local btn = create("TextButton", { Parent = parent, AnchorPoint = Vector2.new(0, 0.5), Position = UDim2.new(0, x, 1, -22), Size = UDim2.new(0, w, 0, 24), BackgroundColor3 = Theme.Bg, AutoButtonColor = false, Text = label, TextColor3 = color, TextSize = 11, Font = Enum.Font.GothamBold, ZIndex = 7 })
    corner(btn, 6); stroke(btn, color, 1, 0.45)
    local active = false
    local function setActive(a)
        active = a
        TweenService:Create(btn, TweenInfo.new(0.12), { BackgroundColor3 = a and color or Theme.Bg, TextColor3 = a and Theme.Bg or color }):Play()
    end
    btn.MouseButton1Click:Connect(function() onClick(not active, setActive) end)
    return setActive
end
local function makeOneShot(parent, label, color, x, w, onClick)
    local btn = create("TextButton", { Parent = parent, AnchorPoint = Vector2.new(0, 0.5), Position = UDim2.new(0, x, 1, -22), Size = UDim2.new(0, w, 0, 24), BackgroundColor3 = Theme.Bg, AutoButtonColor = false, Text = label, TextColor3 = color, TextSize = 11, Font = Enum.Font.GothamBold, ZIndex = 7 })
    corner(btn, 6); stroke(btn, color, 1, 0.45)
    btn.MouseEnter:Connect(function() TweenService:Create(btn, TweenInfo.new(0.12), { BackgroundColor3 = color, TextColor3 = Theme.Bg }):Play() end)
    btn.MouseLeave:Connect(function() TweenService:Create(btn, TweenInfo.new(0.12), { BackgroundColor3 = Theme.Bg, TextColor3 = color }):Play() end)
...
local function fetchPlayers()
    ...
    if ok and json and json.players then
        local newData = {}
        for _, p in ipairs(json.players) do newData[p.user_id] = p end
        playersData = newData
        return true
    end
    return false
end
local function sendCommand(userId, payload) apiPost(BASE_URL .. "/api/command", { user_id = userId, [payload[1]] = payload[2] }) end
local function sendFullState(userId, fps, lagC) apiPost(BASE_URL .. "/api/command", { user_id = userId, fps_limit = fps, lag_c = lagC }) end
function renderPlayers()
    for _, child in ipairs(PlayerContainer:GetChildren()) do if child ~= EmptyLabel and child ~= ListLayout then child:Destroy() end end
    rowsByUser = {}
    ...
    for id, data in pairs(playersData) do
        ...
        if q ~= "" then
            ...
            if data.brainrots then for _, b in ipairs(data.brainrots) do if b.title and b.title:lower():find(q) then brainrotMatch = true; break end end end
            if not (nameMatch or displayMatch or brainrotMatch) then continue end
        end
    ...
    for _, client in ipairs(filtered) do
        local row = create("Frame", { Parent = PlayerContainer, Size = UDim2.new(1, -4, 0, rowHeight()), BackgroundColor3 = Theme.Bg, BorderSizePixel = 0, ClipsDescendants = true, ZIndex = 6 })
        corner(row, 10); stroke(row, Theme.Border, 1, 0.4)
        local dotColor = client.online and Theme.Green or Theme.Gray
        local dot = create("Frame", { Parent = row, Position = UDim2.new(0, 10, 0, 10), Size = UDim2.new(0, 8, 0, 8), BackgroundColor3 = dotColor, BorderSizePixel = 0, ZIndex = 7 })
        corner(dot, 8)
        local nameLbl = create("TextLabel", { Parent = row, Position = UDim2.new(0, 24, 0, 6), Size = UDim2.new(1, -34, 0, 16), BackgroundTransparency = 1, Text = client.display_name or client.username or "?", TextColor3 = Theme.Text, TextSize = 13, Font = Enum.Font.GothamBold, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 7 })
        local subLbl = create("TextLabel", { Parent = row, Position = UDim2.new(0, 24, 0, 22), Size = UDim2.new(1, -34, 0, 14), BackgroundTransparency = 1, Text = ("@%s · id %s · %s"):format(client.username or "?", client.user_id or "?", client.executor or "?"), TextColor3 = Theme.Dim, TextSize = 11, Font = Enum.Font.Gotham, TextXAlignment = Enum.TextXAlignment.Left, TextTruncate = Enum.TextTruncate.AtEnd, ZIndex = 7 })
        local gameLbl = create("TextLabel", { Parent = row, Position = UDim2.new(0, 24, 0, 38), Size = UDim2.new(1, -34, 0, 14), BackgroundTransparency = 1, Text = ("%s · seen %ds ago"):format(client.game_name or "Unknown", math.floor((now - (client.lastHeartbeat or now)) / 1000)), TextColor3 = Theme.Dim, TextSize = 11, Font = Enum.Font.Gotham, TextXAlignment = Enum.TextXAlignment.Left, TextTruncate = Enum.TextTruncate.AtEnd, ZIndex = 7 })
        ...
        setFps = makeChip(row, "FPS", Theme.Orange, 10, 60, function(next, setActive) state.fps_limit = next; setActive(next); sendFullState(userId, state.fps_limit, state.lag_c) end)
        if state.fps_limit then setFps(true) end
        setLagC = makeChip(row, "LAG-C", Theme.Orange, 76, 62, function(next, setActive) state.lag_c = next; setActive(next); sendFullState(userId, state.fps_limit, state.lag_c) end)
        if state.lag_c then setLagC(true) end
        makeOneShot(row, "KICK", Theme.Red, 144, 58, function() sendCommand(userId, { "kick", true }) end)
        local copyBtn = create("TextButton", { Parent = row, AnchorPoint = Vector2.new(0, 0.5), Position = UDim2.new(0, 208, 1, -22), Size = UDim2.new(0, 58, 0, 24), BackgroundColor3 = Theme.Bg, AutoButtonColor = false, Text = "COPY", TextColor3 = Theme.Blue, TextSize = 11, Font = Enum.Font.GothamBold, ZIndex = 7 })
        corner(copyBtn, 6); stroke(copyBtn, Theme.Blue, 1, 0.45)
        copyBtn.MouseEnter:Connect(function() TweenService:Create(copyBtn, TweenInfo.new(0.12), { BackgroundColor3 = Theme.Blue, TextColor3 = Theme.Bg }):Play() end)
        copyBtn.MouseLeave:Connect(function() TweenService:Create(copyBtn, TweenInfo.new(0.12), { BackgroundColor3 = Theme.Bg, TextColor3 = Theme.Blue }):Play() end)
        copyBtn.MouseButton1Click:Connect(function()
            local realUsername = client.username or ""
            if realUsername ~= "" then copyToClipboard(realUsername) end
        end)
...
local function poll()
    if fetchPlayers() then renderPlayers(); updateRowStates() end
end
SearchBox:GetPropertyChangedSignal("Text"):Connect(function() searchQuery = SearchBox.Text; renderPlayers() end)
poll()
task.spawn(function() while ScreenGui.Parent do task.wait(1.5); poll() end end)`;
    res.setHeader('Content-Type', 'text/plain');
    res.send(panel);
});
